...
def encode_image_masked(clip_vision, image, mask=None, batch_size=0, tiles=1, ratio=1.0):
    # full image embeds
    embeds = encode_image_masked_(clip_vision, image, mask, batch_size, image_size=336)
    tiles = min(tiles, 16)
    ...
    if tiles > 1:
        ...
        embeds_split = encode_image_masked_(clip_vision, image_split, mask, batch_size, image_size=336)
...
def encode_image_masked_(clip_vision, image, mask=None, batch_size=0, image_size=336):
    ...
    for img in image_batch:
        ...
        pixel_values = clip_preprocess(img, image_size).float()
